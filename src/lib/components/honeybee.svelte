...
<script lang="ts">
  ...
  export let id: string = "honeybee"; // unique id so the store can tell bees apart
  ...
  function handleClick() {
    ...
    if (clicked) {
      clicked = false;
      vx = (Math.random() > 0.5 ? 1 : -1) * (1 + Math.random()) * 0.85;
      vy = (Math.random() > 0.5 ? 1 : -1) * (1 + Math.random()) * 0.85;
      return;
    }
    clicked = true;
    vx = 0;
...
<style>
  .HoneyBee {
    ...
    background-image: url("/pixel_sheets/honeybee_pixelsheet.png");
